...
const Game4 = {
  ...
  onEnter: function(params) {
    ...
    this.score = 0;
    this.lives = 3;
    this.targetScore = (params && params.targetScore) || 50;
  ...
  update: function(dt) {
    ...
    if (this.score >= this.targetScore) this._onWin();
  },
  ...
  _onWin: function(){ this.ended = true; this.resultMessage = 'You Win!'; },
  _onLose: function(){ this.ended = true; this.resultMessage = 'Game Over'; },
  ...
  render: function(ctx){ ctx.clearRect(0,0,Engine.canvas.width,Engine.canvas.height); ctx.fillStyle='#071227'; ctx.fillRect(0,0,Engine.canvas.width,Engine.canvas.height);
    // player
    ctx.fillStyle='white'; ctx.fillRect(this.player.x,this.player.y,this.player.w,this.player.h);
    // bullets
    ctx.fillStyle='yellow'; for(const b of this.bullets) ctx.fillRect(b.x-2,b.y-6,4,8);
    // enemies
    ctx.fillStyle='#ff6b6b'; for(const e of this.enemies) ctx.fillRect(e.x,e.y,e.size,e.size);
    // HUD
    ctx.fillStyle='white'; ctx.font='18px Arial'; ctx.fillText('Score: '+this.score,12,22); ctx.fillText('Lives: '+this.lives,Engine.canvas.width-110,22);
    // result overlay
